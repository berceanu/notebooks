...
quadψtmom(ky::Float64, kx::Float64; gv=1e-2, vp=(0., 1.9), Δp=0., κ=1.1) = -gv*(ε(ky, kx; Δp=Δp) - (ky*vp[1] + kx*vp[2]) + im*κ)/(ε(ky, kx; Δp=Δp)*(ε(ky, kx; Δp=Δp) + 2)- (ky*vp[1] + kx*vp[2] - im*κ)^2)
